...
```
#### 무슨 원리인가요?
아까 우리는 도커 컴포즈 파일을 템플릿화 해놓았습니다. 그리고 Makefile에서는 `sed` 명령어를 사용하여 사용자가 지정한 id 값과 password를 도커 커포즈 파일에 치환해주고 있답니다.

#### `: instance$(id)` 는 무엇인가요?
Makefile은 한 쌍의 인스턴스를 생성하기 위하여 사용자가 직접 `instance{id}` 에 해당하는 폴더를 생성할 것을 요구하고 있습니다.

그러므로 단순히 `make instance id=3` 라는 명령을 내리는 것으로 3번 인스턴스 쌍이 구동되지 않으니, 같은 폴더에 `instance3` 폴더를 하나 만들어 두세요.

약간의 메뉴얼한 절차를 남겨두어서 볼륨 데이터를 날리는 실수를 방지하기 위함입니다.

#### 인스턴스 마이그레이션은 무엇인가요?
인스턴스 10 쌍을 이용할 것인가요? 그렇다면 젠킨스를 10번 설정하셔야 겠군요?
1. 깃허브 연결
2. 플러그인 설치 등..

이 작업을 10번 반복하는 것은 옳지 않습니다.
그냥 0번 인스턴스에서 모든 것을 설정한 뒤 1~9번 인스턴스로 마이그레이션 하는 것이 좋겠지요?
